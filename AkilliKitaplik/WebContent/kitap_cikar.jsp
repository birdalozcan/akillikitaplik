...
<%
    ...
    if(xx!=null && xx.equals("kapat"))
    {
       ...
       String sql="update tbl_kitap set kitap_aktif=0 where kitap_id='"+kitap_id+"'";
...
<body>
<div class="container">
<form action="kitap_cikar.jsp?kitap=cikar&uye_id=<%=user.getUye_id() %>" method="POST">

    
    <h2>Kitap Çıkar</h2><br>
    <table class="table table-hover">
        <tbody>
            <tr>
                <td>Kitap Adı : </td>
                <td><input class="form-control" type="text" name="kitap_adi"></input></td>
                <td><button type="submit" class="btn btn-success">Kitap Bul</button></td>
            </tr>           
        </tbody>
    </table> 
    </form>
    ...
    if(x!=null && x.equals("cikar"))
    {
       ...
       Connection con= baglanti.connect();
       Statement st = con.createStatement();
       ResultSet rs = st.executeQuery("select * from tbl_kitap where kitap_adi like '%"+kitap_adi+"%' and kitap_aktif=1 ");
       while(rs.next())
       {
...
%>
            <tr>
                <td>Kitap Adı : </td>
                <td><%=kitap_ad%></td>
        <form action="kitap_cikar.jsp?kitap=kapat&kitap_id=<%=kitap_id%>" method="POST" name="form2"><td><input type="submit" value="X" class="btn btn-danger" onClick="myFunction();"></input></td></form>
            </tr>
            <%
       }
       baglanti.close(con);
